export class UserController {
    ...
    async renderUsers(nonTrainedUser) {
        ...
        const existingUser = users.find(user => user.id === nonTrainedUser.id);
        const createdUser = existingUser || await this.#userService.addUser(nonTrainedUser);
        const defaultAndNonTrained = existingUser ? users : [createdUser, ...users];
